...
def chunk_transform_qk_bwd_kernel_prepare(
    ...
    # Load dv (computed externally) and o for dA computation
    p_dv = tl.make_block_ptr(dv, (T, V), (HQ*V, 1), (i_t * BT, 0), (BT, BV), (1, 0))
    b_dv = tl.load(p_dv, boundary_check=(0, 1))
    p_o = tl.make_block_ptr(o, (V, T), (1, HQ*V), (0, i_t * BT), (BV, BT), (0, 1))
    b_ot = tl.load(p_o, boundary_check=(0, 1))

    # dA = -scale * (dV @ O.T) * exp(A * scale - 1/scale + M)
    # M is -inf for upper triangular (including diagonal), 0 for strictly lower triangular
    # This gives strictly lower triangular result
    m_stril = o_i[:, None] > o_i[None, :]  # strictly lower triangular mask
    b_L = tl.where(m_stril, tl.exp((b_A * scale - 1.0 / scale).to(tl.float32)), 0.0).to(b_dv.dtype)
    b_dA = -scale * tl.dot(b_dv, b_ot) * b_L
...
def intra_chunk_preprocess_bwd_prepare_fn(q, k, o, w, beta, A, dv, scale, return_h=True, cu_seqlens=None):
    """
    Backward prepare for intra-chunk preprocessing.
