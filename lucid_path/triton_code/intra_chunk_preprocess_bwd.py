...
def intra_chunk_preprocess_bwd_fn(q, k, w, beta,
                                  ...
                                  A, cu_seqlens=None):
    """
    Backward pass for intra-chunk preprocessing.
    ...
    Args:
        ...
        dk: Gradient of K (input, accumulated)
        dA_local: Gradient of A (computed by intra_chunk_preprocess_bwd_prepare)
        dw1, dw2: Gradients from parallel_path_bwd_intra
        A: Attention matrix (AT)
        cu_seqlens: Cumulative sequence lengths for variable length

    Returns:
        dq_new: Updated gradient of Q
        dk_new: Updated gradient of K
    ...
    # Use num_stages=1 for float32 to reduce shared memory usage
    # Also use different num_warps to force fresh kernel compilation
    is_float32 = (k.dtype == torch.float32)
    intra_chunk_preprocess_bwd_kernel[grid](
        ...
        num_stages=1 if is_float32 else 3,
        num_warps=2 if is_float32 else 4,
    )
